...
export const useFilterByVendor = (vendorId:string) => {
  const {
    filterCategory,
    filterBrand,
    filterColor,
  ...
  const vendorData = products_data?.filter((item)=> item.vendorId == vendorId)
  ...
  const filterByColor = vendorData?.filter(
    (item) =>
      item.productColorArray &&
      item.productColorArray.some((color) => color.color === filterColor)
  );

  const result = filterByColor?.length ? filterByColor : filteredData
  return result;
};
